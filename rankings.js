  ...
  document.getElementById('tableBody').innerHTML = rows.map(d => {
    ...
    return `<div class="ranking-row" onclick="window.location.href='profile.html?ticker=${encodeURIComponent(d.Ticker)}'">
          <div class="row-content">
            ...
            <div class="metric-cell">${metricMarkup(d.Climate_Targets)}</div>
            <div class="metric-cell">${metricMarkup(d.Investment_Transition)}</div>
            <div class="metric-cell">${metricMarkup(d.Climate_Reporting)}</div>
            <div class="metric-cell">${metricMarkup(d.Environment_Score, true)}</div>
            <div class="tier-cell"><span class="tier-pill tier-${tier}">${tierLabel(d.Environment_Score)}</span></div>
          </div>
        </div>`;
  }).join('');
}
